varhi.refine <- function(data.sub,N.core,p.mdl)
{
  
  ########################################################################
  #	varhi.refine by Tucker McElroy, March 9, 2021
  #
  #    Purpose: given core variables and a VAR model order, compute the
  #	impact of each ancillary variable, according to wald tests.
  #	We test the whether each ancillary variable Granger causes the core
  #   variables in the model that includes all core and all ancillary
  #    Inputs: 
  ...
  for(k in (1+length(N.core)):length(N.all))
  {
    E2 <- matrix(diag(length(N.all))[k,],ncol=length(N.all))
    E2 <- t(E2)
    if(p.mdl==1) {
