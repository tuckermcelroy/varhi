varhi.build <- function(data.sub,N.core,p.mdl,ordering,thresh)
{

	########################################################################
	#	varhi.build by Tucker McElroy, August 15, 2017
	#
	#    Purpose: given core variables and a VAR model order, and the wald
	#	statistics for each auxiliary variable, build the initial model
	#	by sequentially adding auxiliary variables to the core, and testing
	#	by Wald against the previous model.
 	#    Inputs: 
	...
	#	ordering: desired ordering of ancillary variables to be entered in the model
  #   so ordering[1] has index of first ancillary variable, 
  #   ordering[2] has the second, etc.
	#	thresh: p-value threshold for rejecting the null of no improvement
	#    Outputs:
	#	N.final: indices of the final model, with core plus the best auxiliaries
	#
	########################################################################		
...
#new.core <- union(new.core,t)
new.core <- union(N.final,t)
N.all <- new.core
...
E2 <- matrix(diag(length(N.all))[length(N.all),],nrow=1) 
E2 <- t(E2)
...
p.val <- 1-pchisq(wald.test,df=length(phi.core.aux))
if(p.val <= thresh) { N.final <- new.core }
if(t == N.best[length(N.best)])
#if((p.val > thresh) || (t == N.best[length(N.best)]))
{
	runit <- FALSE
}
print(c(wald.test,p.val))  } 	# end runit
